...
import { purchases, users, gifts, referralCodes } from "../drizzle/schema";
...
export async function createReferralCode(data: {
  ...
  const db = await getDb();
  if (!db) return;
  await db.insert(referralCodes).values({ ...data, timesUsed: 0 });
}
...
export const stripeRouter = router({
  ...
  /**
   * Get or generate a personal 20% referral code for the current user.
   * Only available after the user has at least one fulfilled purchase.
   * Creates a Stripe coupon + promotion code on first call; returns the
   * existing code on subsequent calls (idempotent).
   */
  getReferralCode: protectedProcedure.query(async ({ ctx }) => {
    // Check if user already has a referral code
    const existing = await getReferralCodeByUserId(ctx.user.id);
    if (existing) {
      return {
        code: existing.code,
    ...
    // Only generate a code if the user has at least one fulfilled purchase
    const userPurchases = await getPurchasesByUserId(ctx.user.id);
    const hasPurchase = userPurchases.some(p => p.status === "fulfilled");
    if (!hasPurchase) {
      return null; // Not eligible yet
    }

    // Generate a unique human-readable code: e.g. "THOMAS20-A3F9"
    const stripe = getStripe();
    const baseName = (ctx.user.name ?? ctx.user.email ?? "FRIEND")
      .toUpperCase()
      .replace(/[^A-Z0-9]/g, "")
      .slice(0, 8);
    const suffix = crypto.randomBytes(2).toString("hex").toUpperCase();
    const code = `${baseName}20-${suffix}`;

    // Create a Stripe coupon (20% off, forever, single-use per customer)
    const coupon = await stripe.coupons.create({
      percent_off: 20,
      duration: "once",
      name: "Refer a Friend — 20% Off",
      metadata: { referrer_user_id: String(ctx.user.id) },
    });

    // Create a Stripe promotion code tied to the coupon
    ...
    return { code, timesUsed: 0, createdAt: new Date() };
  }),
});
