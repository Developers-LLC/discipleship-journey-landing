...
BOOKS = [
    {
        ...
        "cover": "/home/ubuntu/discipleship/kdp_covers/BELONG_KDP_Cover.jpg",
    ...
    },
    {
        ...
        "cover": "/home/ubuntu/discipleship/kdp_covers/GROW_KDP_Cover.jpg",
    ...
    },
    {
        ...
        "cover": "/home/ubuntu/discipleship/kdp_covers/GO_KDP_Cover.jpg",
        "accent": "rgb(13, 31, 60)",
...
def preprocess(text):
    """Strip all code fences (```...```), HTML blocks (<details>...</details>),
    YAML metadata, and other non-body content from the markdown."""
    ...
    # Remove HTML tags
    text = re.sub(r'<[^>]+>', '', text)
    # Remove KDP metadata lines (lines starting with > **Series**: etc.)
    text = re.sub(r'^>\s+\*\*(Series|Level|Focus|Bible Translation)[^\\n]*\n', '', text, flags=re.MULTILINE)
...
def escape_t(s):
    """Escape Typst special chars in a plain string (no markup)."""
    for ch in ['\\', '@', '<', '>']:
        s = s.replace(ch, '\\' + ch)
    s = s.replace('#', '\\#')
    s = s.replace('[', '\\[').replace(']', '\\]')
...
def inline_md(s):
    """Convert inline markdown bold/italic to Typst."""
    # Bold-italic
    ...
    # Escape remaining # not part of Typst commands
    s = re.sub(r'(?<![\\#])#(?![a-z(])', '\\#', s)
...
def md_to_typst_body(md_text, accent):
    ...
    while i < len(lines):
        ...
        m = re.match(r'^##\s+(.+)$', line)
        if m:
            title = escape_t(re.sub(r'\*\*(.+?)\*\*', r'\1', m.group(1).strip()))
        ...
        if m:
            title = escape_t(re.sub(r'\*\*(.+?)\*\*', r'\1', m.group(1).strip()))
            # Detect chapter vs generic section
            if re.match(r'(CHAPTER|FOREWORD|APPENDIX)', title, re.I):
                out.append(f"""
#pagebreak()
#v(2.5em)
...
""")
        ...
        m = re.match(r'^####\s+(.+)$', line)
        if m:
            title = escape_t(re.sub(r'^\d+\.\s+', '', re.sub(r'\*\*(.+?)\*\*', r'\1', m.group(1).strip())))
